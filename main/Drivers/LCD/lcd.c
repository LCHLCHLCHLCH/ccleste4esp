...
void LCD_Fill(u16 xsta, u16 ysta, u16 xend, u16 yend, u16 color)
{
	...
	// t.length = (xend-xsta)*(yend-ysta) * 16;
	// t.tx_buffer = (uint8_t *)&color;
	// t.user = (void *)1;
	// ret = spi_device_polling_transmit(spi_port, &t);
	// assert(ret == ESP_OK);
}
...
void display_all()
{
	...
	// ESP32 LE DMA 低字节先发，ST7735 需高字节先 → 原地交换
...
{
	// rgb565_data 在 flash 里不能原地 swap，用 display_mem 中转
	memcpy(display_mem, rgb565_data, sizeof(display_mem));
	display_all();
}