...
static const uint16_t pico8_rgb565[16] = {
    ...
    0xC007, 0xFD00, 0xFF64, 0x0726,  // 8号改 #C7023D
    0x249D, 0x83B3, 0xFBB5, 0xFE75,  // 12号改 #2492EC
};
...
// 字体数据在 font_data.h 中 (PICO-8 标准 4x6 字体)
        ...
        while (*str) {
            ...
            for (int row = 0; row < 6; row++) {
                ...
                for (int col = 0; col < 4; col++) {
                    if (bits & (1 << (3 - col)))
                        draw_point(x + col - camera_x, y + row - 1 - camera_y, c);
                }
            }
            x += 4;
        }
